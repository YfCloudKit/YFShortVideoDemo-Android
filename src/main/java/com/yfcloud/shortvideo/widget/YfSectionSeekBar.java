    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if (enableShow)
            for (SparseIntArray array : mSectionBackground) {
                ...
                canvas.drawRect((getMeasuredWidth() - getPaddingLeft() - getPaddingRight()) / (float) getMax() * array.get(START_POSITION) + getPaddingRight(), 0, (getMeasuredWidth() - getPaddingLeft() - getPaddingRight()) / (float) getMax() * sectionEndProgress + getPaddingRight(), getMeasuredHeight(), mPaint);
            }
    }

    private boolean enableShow;

    public void enableShowColorBar(boolean enable) {
        enableShow = enable;
    }
