...
public class ScaleGLSurfaceView extends GLSurfaceView {
    ...
    public void initScaleGLSurfaceView(OnScaleCallback mCallback) {
...
//        Log.d(TAG, "onTouchEvent: " + event.getX() + "  " + event.getY());
        ...
        if (action == MotionEvent.ACTION_UP) {
            float xPercent = event.getY() / ((float) getHeight());
            float yPercent = (((float) getWidth()) - event.getX()) /
                    ((float) getWidth());
//                    float xPercent = event.getX() / ((float) mGlSurfaceView.getWidth());
//                    float yPercent = event.getY() / ((float) mGlSurfaceView.getHeight());

            Log.d(TAG, "setAFArea: " + xPercent + " -- " + yPercent);
            Rect focusRect = ScaleGLSurfaceView.calculateTapArea(xPercent, yPercent, 1f);
            Log.d(TAG, "[setAFArea]: " + focusRect.toString());
            if (mCallback!=null) {
                mCallback.onFocus(focusRect);
            }
        }
