...
//                Log.d(TAG,"aTimeList: " + aTimeList);
		...
		if (/*timeList.isEmpty() || */(!timeList.isEmpty() && timeList.getLast() <= Const.MIN_RECORD_TIME)) {
			float left = perWidth * Const.MIN_RECORD_TIME;
			if (canvas != null) {
...
//            if (!timeList.isEmpty()) mRight -= timeList.size() * breakWidth;
    ...
    private float flashWidth = 2f;
    private float minTimeWidth = 2f;
